...
-(void)addObservers
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(willPresentNewPopover:)
                                                 name:@"FPNewPopoverPresented"
                                               object:nil];
...
-(id)initWithViewController:(UIViewController*)viewController
{
    ...
    if(self)
    {
        ...
        __block FPPopoverController *blockSelf = self;
        [_touchView setTouchedOutsideBlock:^{
            [blockSelf dismissPopoverAnimated:YES];
        }];
        
        
        self.contentSize = CGSizeMake(320, 300); //default size
